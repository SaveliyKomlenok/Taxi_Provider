...
public class PassengerController {
    ...
    public ResponseEntity<PassengerListResponse> getAll(@RequestParam(required = false, defaultValue = "0") Integer pageNumber,
        ...
        return new ResponseEntity<>(passengerMapper.fromListEntityToListResponse(passengerList), HttpStatus.OK);
    }

    @GetMapping("/{id}")
    public ResponseEntity<PassengerResponse> getById(@PathVariable Long id) {
        Passenger passenger = passengerService.getById(id);
        return new ResponseEntity<>(passengerMapper.fromEntityToResponse(passenger), HttpStatus.OK);
    }

    @PostMapping
    public ResponseEntity<PassengerResponse> save(@RequestBody @Valid PassengerCreateRequest request) {
        Passenger passenger = passengerService.save(passengerMapper.fromResponseToEntity(request));
        return new ResponseEntity<>(passengerMapper.fromEntityToResponse(passenger), HttpStatus.CREATED);
    }

    @PutMapping
    public ResponseEntity<PassengerResponse> update(@RequestBody @Valid PassengerUpdateRequest request) {
        Passenger passenger = passengerService.update(passengerMapper.fromResponseToEntity(request));
        return new ResponseEntity<>(passengerMapper.fromEntityToResponse(passenger), HttpStatus.OK);
    }
