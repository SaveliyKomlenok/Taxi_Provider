...
public class DriverRatingController {
    ...
    public Mono<ResponseEntity<DriverRatingResponse>> getDriverRating(@PathVariable Long id) {
        return driverRatingService.getByDriverId(id)
                .map(driverRating -> ResponseEntity.ok(driverRatingMapper.toResponse(driverRating)))
                .defaultIfEmpty(ResponseEntity.notFound().build());
    }

    @PostMapping
    @PreAuthorize("hasRole('passenger')")
    public Mono<ResponseEntity<DriverRatingResponse>> save(@RequestBody @Valid DriverRatingRequest request) {
        return driverRatingService.save(driverRatingMapper.toEntity(request))
                .map(driverRating -> ResponseEntity.ok(driverRatingMapper.toResponse(driverRating)));
    }
}